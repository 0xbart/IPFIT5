...
db = sqlite3.connect('testingaaa.db') #### NOG AANPASSEN, OOK DE SQL STATEMENTS
cursor = db.cursor()
...
html += '''
<table class="table">
<tr><th>ID</th><th>Naam</th><th>Beschrijving</th><th>Type</th><th>Aangemaakt op:</th><th>Verwijderd</th></tr>'''
...
for row in table:
     html += ('<tr>')
     html += ('<td>' '{0}''</td> ' '<td>' '{1}' '</td> ' '<td>' '{2}' '<td>' '{3}''<td>' '{4}''<td>' '{5}'.format(row[0], row[1], row[2], row[3], row[4], row[5]) + '</td>')
     html += ('</tr>')
...
html += '''
<table class="table">
<tr><th>ID</th><th>Naam</th><th>Beschrijving</th><th>Aangemaakt op:</th></tr>'''
...
for row in table:
     html += ('<tr>')
     html += ('<td>' '{0}''</td> ' '<td>' '{1}' '</td> ' '<td>' '{2}' '<td>' '{3}'.format(row[0], row[1], row[2], row[3]) + '</td>')
     html += ('</tr>')
...
cursor.execute('''SELECT id, processor, usb_devices, system_arch, proc_name, proc_family, used_memory, free_memory, total_memory FROM laptop_hardware''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, name FROM laptop_software''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, dropbox, onedrive, evernote, googledrive FROM laptop_cloud''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, his_chrome, his_ff, his_iexplorer FROM laptop_browser''')
table = cursor.fetchall() #retrieve the first row
...
html += '''
<table class="table">
<tr><th>ID</th><th>Drive Naam:</th><th>Drive Detail:</th></tr>'''

cursor.execute('''SELECT id, drive_name, drive_detail FROM laptop_drive''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, name, parent, shahash, md5hash FROM laptop_files''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, name FROM laptop_linux_logon''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, name FROM laptop_users''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, name FROM laptop_win_logon''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, virus_name, virus_hash, virus_output FROM laptop_virus''')
table = cursor.fetchall() #retrieve the first row
...
cursor.execute('''SELECT id, ip, mac, connected_ip FROM laptop_network''')
table = cursor.fetchall() #retrieve the first row
